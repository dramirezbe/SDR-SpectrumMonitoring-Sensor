...
void hackrf_apply_cfg(hackrf_device* dev, SDR_cfg_t *cfg) {
    ...
    // Keep corrected center frequency available for DSP/logging metadata.
    double correction = 1.0 + ((double)cfg->ppm_error / 1000000.0);
    cfg->center_freq_corrected = (uint64_t)((double)cfg->center_freq * correction);
