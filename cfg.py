...
class AtomicRotator:
    """
    Gestor de logs que utiliza atomic_write_bytes para proteger la integridad.
    """
    ...
    def _generate_path(self) -> pathlib.Path:
        return LOGS_DIR / f"{get_time_ms()}_{self.module_name}.log"
...
class Tee:
    """Redirige stderr al gestor atómico sin afectar stdout."""
...
def set_logger(rotator: AtomicRotator | None = None) -> logging.Logger:
    ...
    Consola: INFO (si VERBOSE) / ERROR.
    Archivo: WARNING o superior (Atomic).
    """
...
def run_and_capture(func: TargetFunc, num_files=LOG_FILES_NUM) -> int:
    ...
    log = set_logger(rotator)
    
    orig_err = sys.stderr
    rc = 1

    try:
        # Solo stderr se duplica al archivo para capturar errores críticos/tracebacks
...
if __name__ == "__main__":
    def debug_test():
        l = set_logger()
        l.info("Info test - Only console if verbose")
        l.warning("Warning test - Console and File")
        return 0
